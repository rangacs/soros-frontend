...
const useStyles = makeStyles((theme) => ({
    loading: {
        ...
        alignItems: 'center',
        width: '100%'
        // height: '100vh' // This will make it centered vertically on the whole viewport. Adjust as needed.
    },
    root: {
        width: '100%'
    },
...
export default function EnhancedTable({ settings, tagStartTime, tagEndTime }) {
    ...
    React.useEffect(() => {
        ...
        let startTime = historicalData === 'yes' ? format(historicalStart, 'Y-MM-d HH:mm') : format(rangeStart, 'Y-MM-d HH:mm');
        let endTime = historicalData === 'yes' ? format(historicalEnd, 'Y-MM-d HH:mm') : format(rangeEnd, 'Y-MM-d HH:mm');

        try {
            startTime = tagStartTime.slice(0, -3);
            endTime = tagEndTime.slice(0, -3);
        } catch (e) {
            console.error(e);
        }
        analysisService
    ...
    }, [intervalHour, page, rowsPerPage, settings.settings, tagEndTime, tagStartTime]);
