...
import SetpointsAdd from './SetpointsAdd';
...
const headCells = [
    ...
    },
    {
        ...
        disablePadding: false,
        label: 'Tolerance',
        columnName: 'sp_tolerance_ulevel'
    },
    {
        id: 'sp_status',
        numeric: false,
...
const SetpointsManagement = () => {
    ...
    const getData = React.useCallback(() => {
        SetpointsService.getAllSetpoints()
            .then((res) => {
                if (res && res.data) {
                    const data = res.data;
                    setRows(data);
    ...
    const deleteRecord = async (id) => {
        SetpointsService.deleteSetpoints(id)
            .then((res) => {
                if (res.message) {
    ...
    return (
        <Grid item xs={12} sm={6} lg={12} spacing={gridSpacing}>
            <MainCard style={{ padding: 0 }} title="Setpoints" darkTitle=" ">
                    ...
                    <CardContent>
                        <Grid container justifyContent="space-between" alignItems="center" spacing={2}>
                            ...
                            <Grid item xs={12} sm={6} sx={{ textAlign: 'right' }}>
                                ...
                                {open && <SetpointsAdd id={id} open={open} handleCloseDialog={() => setOpen(!open)} getData={getData} />}
                            </Grid>
                        </Grid>
                    </CardContent>
                </Box>
                                            ...
                                            >
                                                ...
                                                <TableCell align="center" sx={{ pr: 3 }}>
                                                    <IconButton component={Button}>
                                                        <IconEdit onClick={() => editHandler(row.sp_id)} />
                                                    </IconButton>
                                                    <IconButton
                                                        aria-label="delete"
                                                        onClick={() => {
                                                            setConfirmOpen(true);
                                                            setDeleteId(row.sp_id);
                                                        }}
                                                    >
                                                        <DeleteIcon />
                                                    </IconButton>
                                                    <div>
                                                        ...
                                                        >
                                                            <DialogTitle id="alert-dialog-title">Delete RmSetpoint</DialogTitle>
                                                            <DialogContent>
                                                                <DialogContentText id="alert-dialog-description">
                                                                    Do you want to delete the RmSetpoint ?
                                                                </DialogContentText>
                                                            </DialogContent>
                                                            <DialogActions>
...
export default SetpointsManagement;
