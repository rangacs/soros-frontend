...
import { DEFAULT_COLUMNS, DEFAULT_ROWS } from '../../../_helpers/default-data';
...
export default function EnhancedTable(props) {
    ...
    React.useEffect(() => {
                    ...
                    .then(async (results) => {
                        ...
                        /* if (data.length && data[0]) {
                            ...
                            for (const value of Object.keys(data[0])) {
                                const findColumn = tags.find((obj) => obj.columnName === value);
        ...
        /*  const myRefreshInterval = setInterval(() => {
            if (new Date().getTime() > deactivateRefresh && historicalData !== 'yes') {
                getData();
            }
    ...
    const getFilteredValue = (ele, value) => {
        if (ele.columnName === 'LocalstartTime' || ele.columnName === 'LocalendTime') {
            return format(new Date(value ?? 0.0), 'Y-MM-d HH:mm');
        }
