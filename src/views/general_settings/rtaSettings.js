...
import MonitorColumnAdd from './MonitorColumnAdd';
...
const MonitorColumnManagement = () => {
    ...
    const getData = React.useCallback(() => {
        MonitorColumnService.getAllMonitorColumn()
            .then((res) => {
                if (res && res.data) {
                    const data = res.data;
                    setRows(data);
    ...
    const deleteRecord = async (id) => {
        MonitorColumnService.deleteMonitorColumn(id)
            .then((res) => {
                if (res.message) {
        ...
        <>
            <Grid container justifyContent="space-between" alignItems="center" spacing={2}>
                ...
                <Grid item xs={12} sm={6} sx={{ textAlign: 'right' }}>
                    ...
                    {open && <MonitorColumnAdd id={id} open={open} handleCloseDialog={() => setOpen(!open)} getData={getData} />}
                </Grid>
            </Grid>
...
export default MonitorColumnManagement;
